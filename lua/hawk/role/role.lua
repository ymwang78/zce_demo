...
local function _getRoleIdLevel(roleid)
	c.log(1, " ", "_getRoleIdLevel:", roleid)
...
local function _getRoleIdUpperLevel(roleid)
	c.log(1, " ", "_getRoleIdUpperLevel:", roleid)
	local level = _getRoleIdLevel(roleid)
	local uproleid = roleid & (0xffffffffffffffff >> ((8-(level - 1)) * 8))
	c.log(1, "\t", "_getRoleIdUpperLevel:", level, string.format("%x", roleid), string.format("%x", uproleid))
	return uproleid
end

-- 查看这个是否本级管理员，例如 0x011234 得到  true
local function _isRoleIdAdmin(roleid)
	c.log(1, " ", "_isRoleIdAdmin:", roleid)

	local level = _getRoleIdLevel(roleid)
	return (roleid >> ((level - 1) * 8) & 0xff) ==  1
end

local function _getNextRoleId(orgid, father_roleid)
	c.log(1, " ", "_getNextRoleId:", father_roleid)
	...
	-- 	c.log(1, "\t", "_check_default_role:", level, string.format("%x", public_roleid), string.format("%x", admin_roleid))
	...
	-- end
	local mask = 0xffffffffffffffff >> ((7 - level) * 8)
	local father_mask = 0xffffffffffffffff >> ((8 - level) * 8)
	...
	c.log(1, "\t", "_getNextRoleId", string.format("%x", mask), string.format("%x", father_mask), orgid, string.format("%x", father_roleid)) -- string.format("%x", mask) res[1].roleid
	-- c.log(1, "\t", "_getNextRoleId", c.tojson(res)) -- string.format("%x", mask) res[1].roleid
...
function _M.getRole2(orgid, roleid)
	...
	if (_ROLES[orgid] ~= nil) then
		...
		local ok, res = c.rdb_query(pgdb, "select * from roles where orgid = ? and roleid = ? order by roleid", orgid, roleid)
	...
	local ok, res = c.rdb_query(pgdb, "select * from roles where orgid = ? order by roleid", orgid)
	if not ok or #res == 0 then
		return false, nil
	end
	local org_roles = {}
	for i = 1, #res do
		...
		c.log(1, " ", "getRole2:", orgid, c.tojson(res[i], true))
	end
...
function _M.checkRoleRoot(orgid, roleid, owneriid)
	c.log(1, " ", "checkRoleRoot:", roleid)
	local level = _getRoleIdLevel(roleid) -- 0x1234
	local public_roleid = roleid
	local admin_roleid = roleid + (1 << (level * 8))

	local ok, res =  c.rdb_query(pgdb, "select count(*) as rownum from roles where roleid = ?", public_roleid)
	...
	--[[
	local ok, res =  c.rdb_query(pgdb, "select count(*) as rownum from roles where roleid = ?", admin_roleid)
	if (ok and res[1].rownum < 1) then
		local ok, res = c.rdb_query(pgdb, 
			"insert into roles(orgid, roleid, owneriid, rolename, roledesc) values(?, ?, ?, ?, ?)",
...
function _M.addRole(orgid, father_roleid, owneriid, name, desc)
	...
	local ok, res = c.rdb_query(pgdb, "insert into roles(orgid, roleid, owneriid, rolename, roledesc) values(?, ?, ?, ?, ?)",
...
function _M.updateRole2(orgid, roleid, owneriid, rolename, roledesc)
	...
	local ok, res = c.rdb_query(pgdb, "update roles set owneriid=?, rolename=?, roledesc=? where orgid=? and roleid=?",
...
function _M.deleteRole(orgid, roleid)
	...
	local ok, res = c.rdb_query(pgdb, "delete from roles where orgid=? and roleid=?",
		orgid, roleid)
...
local function _getUserOrgs(objtable, iid)
	...
	local ok, res = c.rdb_query(pgdb, 
		"select orgid from roles_orgs where owneriid = ? and orgid in (select DISTINCT orgid from " .. objtable .. ")", iid)
	if not ok then
		return false, "dbquery failed"
	end
...
function _M.getRoleChildrenIds(orgid, roleid)
	c.log(1, " ", "getRoleChildrenIds: ", "orgid:" .. orgid, "roleid:" .. string.format("0x%x", roleid))
	
	--if (not _isRoleIdAdmin(roleid)) then -- 0x011234
	--	c.log(1, "\t", "getRoleChildrenIds not admin:", orgid, string.format("%x", roleid))
	--	return {}
	--end

	--roleid = _getRoleIdUpperLevel(roleid)  -- 0x1234
	--if _ROLE_CHILDREN[roleid] ~= nil then 
	--	c.log(1, "\t", "getRoleChildrenIds:", orgid, roleid, _ROLE_CHILDREN[roleid])
	...
	if (roleid == 0) then
		ok, res = c.rdb_query(pgdb, "select roleid from roles where orgid = ? order by roleid", orgid)
		c.log(1, "\t", "getRoleChildrenIds getAll: ", "orgid:" .. orgid, "reslen:" .. #res)
	else
		local level = _getRoleIdLevel(roleid)
		local next_roleid = roleid + (1 << (level * 8)) -- 0x3512
		c.log(1, "\t", "getRoleChildrenIds:", string.format("%x", roleid),  string.format("%x", next_roleid))
		ok, res = c.rdb_query(pgdb, "select roleid from roles where orgid = ? and roleid >= ? and roleid < ? order by roleid", 
...
function _M.getRoleRightAllItemDict(objtable, orgid, roleid, aclitem, objitem_array)
	c.log(1, " ", "getRoleRightAllItemDict:", objtable, orgid, roleid, aclitem)
	-- global admin, just allow everything
	if (_isRoleIdAdmin(roleid) and _getRoleIdUpperLevel(roleid) == 0) then
		local children = _M.getRoleChildrenIds(orgid, 0)
...
-- 查询用户在该组织的所有角色
function _M.getUserRoleIds(iid, orgid)
    c.log(1, " ", "getUserRoleIds:", iid, orgid)
...
function _M.canAdminRole(iid, orgid, roleid)
	...
	local roleids = _M.getUserRoleIds(iid, orgid)
	for i = 1, #roleids do
...
function _M.getUserOrgRoles2(iid, orgid, roles)
	local set_root_admin = false
	local org = hro.getOrg(orgid)
	if (org ~= nil and org.owneriid == iid) then
		local ok, role = _M.getRole2(orgid, 1)
		roles[#roles + 1] = role
		set_root_admin = true
	end
	...
	local roleids = _M.getUserRoleIds(iid, orgid)
	for i = 1, #roleids do
		if set_root_admin and roleids[i] == 1 then
		else
...
function _M.addUserRole(creatoriid, iid, orgid, roleid)
	...
	local roles = _M.getUserRoleIds(iid, orgid)
...
function _M.getUserRoleRightAllitem(objtable, iid, orgid, aclitem, objitem_array)
	...
	if (iid == org.owneriid) then 
		...
		roleids = _M.getUserRoleIds(iid, orgid)
		if (roleids == nil) then
			return objitem_array
		end
	end
...
function _M.getUserAllRoles2(objtable, iid)
	...
	c.log(1, "\t", c.tojson(orgids, true))
	local roles_array = {}
	for k in pairs(orgids) do
		_M.getUserOrgRoles2(iid, k, roles_array)
	end
	return ok, roles_array
end
...
function _M.doTestRole(useriid, orgid)
	...
	local roles = _M.getUserRoleIds(useriid, orgid)
